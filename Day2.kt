...
fun day2Part1(input: String): Int {
	val maxMap = mapOf("red" to 12, "blue" to 14, "green" to 13)
	val lines = input.split("\n")
	val games = lines.map { parseGameData(it) }
	return games.filter { it.second.firstOrNull { map -> map.count { entry -> entry.value > maxMap[entry.key]!! } > 0 } == null }.sumOf { it.first }
}

fun day2Part2(input: String): Long {
	val lines = input.split("\n")
	val games = lines.map { parseGameData(it).second }
	val maxes = games.map { mergeMaps(it) }
	return maxes.sumOf { it.values.fold(1) { product, value -> product * value }.toLong() }
}

fun mergeMaps(maps: List<Map<String, Int>>): Map<String, Int> {
	return maps.reduce { acc, map ->
		map.keys.union(acc.keys).associateWith { key ->
			maxOf(map[key] ?: 0, acc[key] ?: 0)
		}
	}
}


fun parseGameData(input: String): Pair<Int, List<Map<String, Int>>> {
	// Splitting on colon
	val parts = input.split(":")
	val gameNumber = parts[0].trim().removePrefix("Game").trim().toInt()

	// Splitting on semicolons
	val segments = parts[1].split(";").map { it.trim() }

	val results = mutableListOf<Map<String, Int>>()

	for (segment in segments) {
		val counts = mutableMapOf("red" to 0, "blue" to 0, "green" to 0)

		// Splitting each segment into color parts
		segment.split(",").forEach {
			val colorParts = it.trim().split(" ")
			if (colorParts.size == 2) {
				val color = colorParts[1]
	...
	return Pair(gameNumber, results)
}

// maximum: 12 red cubes, 13 green cubes, and 14 blue cubes, sum IDs of games

const val day2TestInput = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green"""

const val day2PuzzleInput = """Game 1: 1 blue; 4 green, 5 blue; 11 red, 3 blue, 11 green; 1 red, 10 green, 4 blue; 17 red, 12 green, 7 blue; 3 blue, 19 green, 15 red
