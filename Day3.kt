...
fun main() {
	println(day3Part1(day3TestInput))
	println(day3Part1(day3PuzzleInput))
	println(day3Part2(day3TestInput))
	println(day3Part2(day3PuzzleInput))
}

fun day3Part1(input: String): Int {
	return sumPartNumbers(input.split("\n"))
}

fun day3Part2(input: String): Int {
	return sumGearRatios(input.split("\n"))
}

fun sumGearRatios(schematic: List<String>): Int {
			...
			if (schematic[i][j] == '*') {
				...
				for (di in deltas) {
					for (dj in deltas) {
						...
						val adjI = i + di
						val adjJ = j + dj
						if (adjI in schematic.indices && adjJ in schematic[adjI].indices && schematic[adjI][adjJ].isDigit()) {
							partNumbers.add(extractNumber(schematic, adjI, adjJ))
						}
					}
				}

				if (partNumbers.size == 2) {
					val product = partNumbers.map { it.toInt() }.reduce { acc, num -> acc * num }
					sumOfRatios += product
				}
			}
		}
	...
	for (i in schematic.indices) {
		for (j in schematic[i].indices) {
			if (!schematic[i][j].isDigit() && schematic[i][j] != '.') {
				symbols.add(schematic[i][j])
				val partNums = mutableSetOf<String>()
				for (di in deltas) {
					for (dj in deltas) {
						if (di == 0 && dj == 0) continue
						val adjI = i + di
						val adjJ = j + dj
						if (adjI in schematic.indices && adjJ in schematic[adjI].indices && schematic[adjI][adjJ].isDigit()) {
							partNums.add(extractNumber(schematic, adjI, adjJ))
						}
					}
				}
				partNumbers.add(partNums.sumOf { it.toInt() })
			}
		}
	}
...
fun extractNumber(schematic: List<String>, i: Int, j: Int): String {
	...
	while (j - offset >= 0 && schematic[i][j - offset].isDigit()) {
		num = schematic[i][j - offset] + num
		offset++
	}

	// Reset offset and check right for digits
	offset = 1
	while (j + offset < schematic[i].length && schematic[i][j + offset].isDigit()) {
		num += schematic[i][j + offset]
		offset++
	}
...
const val day3TestInput = """467..114..
...*......
..35..633.
......#...
617*......
...
const val day3PuzzleInput = """.......12.......935............184.720...243........589.652..........435..........483.............6...........................904...........
